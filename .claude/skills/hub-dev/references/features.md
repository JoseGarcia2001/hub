...
- **Sparklines semanales por acción** (`prices.ts`, `getWeeklyCloses(positions)`): trae ~5 cierres diarios por ticker de **Yahoo Finance** (endpoint `chart`, sin API key), validado con Zod y cacheado en un Map de proceso (TTL 12 h — el hub es un contenedor de larga vida). `toYahooSymbol` mapea el símbolo IBKR: US tal cual, HKD numérico → `.HK`, cripto (`BTC.USD-PAXOS`) → `BTC-USD`. Best-effort: un ticker que falle queda sin sparkline, nunca rompe la página. La UI (`PortfolioOverview`) muestra una **tarjeta por posición** (reemplaza la tabla): % de 7 días, sparkline SVG a mano (línea punteada en `avgCost`, solo si Yahoo reporta la misma moneda nativa), precio, tu costo, valor y P&L. Self-check: `pnpm --filter @hub/core selfcheck:prices`.
